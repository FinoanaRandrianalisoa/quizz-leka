...
def _pg_only(sql, reverse_sql=migrations.RunSQL.noop):
    """SQL brut réservé à Postgres (Railway).

    Sur SQLite (tests), le schéma est déjà créé par les migrations « propres »
    (AddField / CreateModel) : ce SQL idempotent est ignoré pour rester compatible.
    """
